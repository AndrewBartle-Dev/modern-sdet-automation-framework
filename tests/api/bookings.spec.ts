...
test.describe("Bookings API", () => {
  ...
  test.describe("POST /bookings", () => {
    test(
      "creates booking and returns 201 with valid schema",
      ...
      async ({ userClient }) => {
        ...
        await userClient.events.delete(body.data.eventId);
        
      },
    );

    //TODO: There is some flakiness with this test, so skipping for now.
    //Need to come back to debug seat decrement expect.
    test.skip(
      "booking decrements available seats on the event",
      { tag: ["@api", "@regression"] },
      async ({ userClient }) => {
        const event = await createTestEvent(userClient);
        ...
        await userClient.events.delete(updatedBody.data.id);
      },
    );

    test(
      "returns 400 when quantity exceeds available seats",
      { tag: ["@api", "@regression"] },
      async ({ userClient }) => {
        const event = await createTestEvent(userClient);
        ...
        await userClient.events.delete(event.id);
      },
    );

    test(
      "returns 400 when quantity is 0",
      { tag: ["@api", "@regression"] },
      async ({ userClient }) => {
        const event = await createTestEvent(userClient);
  ...
  test.describe("GET /bookings", () => {
    test(
      "returns paginated bookings list with valid schema",
      { tag: ["@smoke", "@api", "@regression"] },
      async ({ userClient }) => {
  ...
  test.describe("GET /bookings/:id", () => {
    test(
      "returns single booking with valid schema",
      { tag: ["@api", "@regression"] },
      async ({ userClient }) => {
        const event = await createTestEvent(userClient);
  ...
  test.describe("GET /bookings/ref/:ref", () => {
    test.skip(
      "returns booking by reference code",
      { tag: ["@api", "@regression"] },
      async ({ userClient }) => {
        const event = await createTestEvent(userClient);
        const created = await userClient.bookings.create({
    ...
    test(
      "returns 404 for unknown reference",
      { tag: ["@api", "@regression"] },
      async ({ userClient }) => {
  ...
  test.describe("DELETE /bookings/:id", () => {
    ...
    //Need to come back to debug seat decrement expect.
    test.skip(
      "cancels booking and restores seats to event",
      { tag: ["@smoke", "@api", "@regression"] },
      async ({ userClient }) => {
        const event = await createTestEvent(userClient);
        const quantity = 3;
        ...
        const afterBookingBody = (await afterBooking.json()) as Record<
          string,
          any
        >;
        expect(afterBookingBody.data.availableSeats).toBe(
        ...
        const afterCancelBody = (await afterCancel.json()) as Record<
          string,
          any
        >;
